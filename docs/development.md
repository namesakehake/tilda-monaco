...
2. Увеличьте `@version` в `src/tilda-monaco.user.js`, например с `1.1.12` до `1.1.13`. Это нужно для каждого публичного обновления, в том числе только CSS. Без увеличения версии Tampermonkey не обнаружит новый релиз.
...
В установочный файл также включаются тексты `LICENSE` и `THIRD_PARTY_NOTICES`. При распространении сохраняйте их; сторонние компоненты не меняют свою лицензию на MIT.
